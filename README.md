...
- **Incremental indexing.** By default, `POST /ai/rag/index` skips articles whose `updatedAt` matches the timestamp of their existing vectors. Only changed or never-indexed articles are re-embedded, which significantly reduces Gemini quota usage on large collections. To force a full reindex (e.g. after changing chunk size or embedding model), pass `{"force": true}` in the request body. The response includes `skippedArticles` count.
